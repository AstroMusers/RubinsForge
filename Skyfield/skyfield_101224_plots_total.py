...
sat_nums = []
day_nums = []
#read_dirs = ['/data/a.saricaoglu/Lumos-Sat/Files/01.25/2230', '/data/a.saricaoglu/Lumos-Sat/Files/01.27/1949','/data/a.saricaoglu/Lumos-Sat/Files/01.25/2212','/data/a.saricaoglu/Lumos-Sat/Files/01.27/1947']
# file_dirs = ['/2230_satellite_data.fits','1949_satellite_data.fits','/2212_satellite_data.fits','/1947_satellite_data.fits']
read_dirs = ['/data/a.saricaoglu/Lumos-Sat/Files/10_days_0.6s/', '/data/a.saricaoglu/Lumos-Sat/Files/10_days_6s/']
# for read, file in zip(read_dirs, file_dirs):
for dir in read_dirs:

    runs= [x[1] for x in os.walk(dir)][0]
    print(runs)
    for run in runs:
        out = [f for f in os.listdir(dir + run) if ".fits" in f]
        print(out)
        with fits.open(dir + run + '/satellite_data.fits') as hdul:
            hdul.info()
    ...
    read_dirs = [dir + run for run in runs]
    ...
    a4 = [int(ev / 3) for ev in total_event_all]
    ...
    plt.legend()
    # Show the plot
    plt.savefig(directoryp +  "/" + c.strftime('%H%M') + "/Event_histogram_for_"  + str(obs_start.astimezone(zone)) +  f"{dir.split('/')[-2]}.png ")
    ...
    #     plt.savefig(directoryp +  "/" + c.strftime('%H%M') + "/Piechart_for_"  + str(obs_start.astimezone(zone)) +  f"{dir.split('/')[-2]}.png ")    
    ...
    plt.savefig(directoryp +  "/" + c.strftime('%H%M') + "/Brightness_histogram_for_" + str(obs_start.astimezone(zone)) +  f"{dir.split('/')[-2]}.png ")
    ...
    # Show the plot
    plt.savefig(directoryp +  "/" + c.strftime('%H%M') + "/Separation_histogram_for_" + str(obs_start.astimezone(zone)) +  f"{dir.split('/')[-2]}.png ")
