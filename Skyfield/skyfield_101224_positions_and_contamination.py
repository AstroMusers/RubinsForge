...
hdu_pr.header['INFO'] = f'Satellite (n = {Nsats}) positions and brightness for 0.1m intervals for master.fits targets'
...
while t00 < obs_end:
    ...
    for target in targets:
        astro = rubinobs_astr.at(t00).observe(target)
        app = astro.apparent()
        ra_t, dec_t, distance_t = app.radec()
    ...
    for starlink in starlinks[:Nsats+1]:
        ...
        for rise_t, peak_t, set_t in zip(rising_time,peak_time, setting_time):
            ...
            print('sunlit ?', sunlit, 'night ?', night)
            ...
            if night & sunlit:
                ...
                while (ti < set_t) & ((ti < day_start) | (ti > day_end)) & starlink.at(ti).is_sunlit(eph):
                    ...
                    separation = []
                    for target in targets:    
                        astro = rubinobs_astr.at(ti).observe(target)
                        app = astro.apparent()
                        difference_angle = topocentric.separation_from(app)
            ...
            print('no transit rising time ', rise_t.astimezone(zone))
            print('no transit setting time ', set_t.astimezone(zone))   
            # if len(separation) != 0:       
    ...
    position_hdu.header['VALIDATED'] = valid_starlink_counter
    position_hdu.header['INVALIDATED'] = invalid_starlink_counter
    position_hdu.header['EVENTS'] = event_counter
    position_hdu.header['TRAIL_CANDIDATES'] = trailcan_counter
    position_hdu.header['CONTAMINATED'] = contamination_counter
    position_hdu.header['CLOSEST_APPROACH'] = np.min(daily_separation)
