...
with fits.open(fit_filename) as hdu:
    hdu_pr.header['RUNDATE'] = c.strftime('%Y-%m-%d')
    hdu_pr.header['OBSSTART'] = str(obs_start)
    hdu_pr.header['OBSEND'] = str(obs_end)  
    hdu_pr.header['INFO'] = f'Satellite (n = {Nsats}) positions and brightness for 0.1m intervals between {obs_start} and {obs_end} for master.fits targets'
    hdu.append(hdu_pr)
...
while t00 < obs_end:
    ...
    position_hdu = fits.BinTableHDU(data=[sat_t, sat_ra, sat_dec, sat_dist], names=['Time', 'RA', 'Dec', 'Distance'])
