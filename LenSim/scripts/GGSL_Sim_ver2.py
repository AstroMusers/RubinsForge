# only alpha rs, theta_e, rs and rc are randomized.
# # SimulationAPI
...
for k in range(0,model_num):
    ...
    kwargs_pemd = {
        'theta_E': np.random.uniform(0.8, 3),'gamma': 1.95, 'center_x': 0.0, 'center_y': 0, 'e1': 0.1, 'e2': 0.1}  # parameters of the deflector lens model
    kwargs_nfw = {
        'alpha_Rs': 0.02,
        'r_core': np.random.uniform(0.01, 0.2),
        'Rs': np.random.uniform(0.2, 0.3),
        'center_x': 1.,
        'center_y': 1.
    }
    ...
    fixed_lens.append({})
    kwargs_lens_init.append({'alpha_Rs': 0.01, 'r_core' : 0.05, 'Rs': 0.2, 'center_x': 0, 'center_y': 0})
    ...
    mcmc_new_list = []
    labels_new = [r"$x_{clump}$", r"$y_{clump}$", r"$R_s$", r"$\alpha_{Rs}$", r"$R_{core}$", r"$\theta_E$"]
